...
def get_existing_video_ids():
    ...
    if os.path.exists("akcija.xml"):
        try:
            tree = ET.parse("akcija.xml")
            root = tree.getroot()
            
            # Proveri da li je category element
            if root.tag == 'category':
                ...
                # Stari format sa movies elementom
                for movie in root.findall('movie'):
                    video_id_elem = movie.find('videoId')
                    if video_id_elem is not None and video_id_elem.text:
                        existing_ids.add(video_id_elem.text)
            
            print(f"📁 Pronađeno {len(existing_ids)} postojećih videoId-eva u akcija.xml")
        except Exception as e:
...
def save_formatted_xml(tree, output_file):
    """Čuva XML u lepo formatiranom obliku SA CATEGORY"""
    root = tree.getroot()
    
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write('<?xml version="1.0" encoding="UTF-8"?>\n')
        f.write('<category name="Akcija">\n')
...
def update_genre_to_akcija():
    """Ažurira genre za sve filmove na 'Akcija'"""
    if not os.path.exists("akcija.xml"):
    ...
    try:
        ...
        print("🔄 Ažuriram žanr za sve filmove na 'Akcija'...")
        ...
        for movie in movies_parent.findall('movie'):
            genre_elem = movie.find('genre')
            if genre_elem is not None:
                genre_elem.text = "Akcija"
                updated_count += 1
            else:
                # Ako ne postoji genre element, kreiraj ga
        ...
        if updated_count > 0:
            ...
            save_formatted_xml(tree, "akcija_updated.xml")
            print(f"\n🎉 Ažurirano {updated_count} žanrova u akcija_updated.xml")
        else:
            print("ℹ️  Nema žanrova za ažuriranje")
            
    except Exception as e:
        print(f"❌ Greška pri ažuriranju žanrova: {e}")

def copy_existing_movies_to_category(new_root):
    """Kopira postojeće filmove u novi category element i ažurira genre na 'Akcija'"""
    if not os.path.exists("akcija.xml"):
        return 0
    
    try:
        existing_tree = ET.parse("akcija.xml")
        existing_root = existing_tree.getroot()
        
        copied_count = 0
        
        # Proveri da li je category element
        if existing_root.tag == 'category':
            movies_parent = existing_root
        else:
            movies_parent = existing_root
        
        # Kopiraj sve postojeće filmove i ažuriraj genre
        for movie in movies_parent.findall('movie'):
            # Ažuriraj genre na "Akcija" pre kopiranja
            genre_elem = movie.find('genre')
            if genre_elem is not None:
                genre_elem.text = "Akcija"
            else:
        ...
        print(f"📋 Kopirano {copied_count} postojećih filmova (žanr ažuriran na 'Akcija')")
        return copied_count
...
def create_xml_with_translation():
    ...
    playlist_url = "https://www.youtube.com/playlist?list=PLFTGssSZfmnRW6Qj4gO0PyUxQeUKY-4B2"
    ...
    # Kreiraj NOVI XML root SA CATEGORY
    new_root = ET.Element("category")
    new_root.set("name", "Akcija")
    
    # Inicijalizuj translator
    translator = RobustTranslator()
    
    # Prvo kopiraj SVE postojeće filmove iz starog XML-a i ažuriraj genre
    existing_count = copy_existing_movies_to_category(new_root)
    ...
    for i, video in enumerate(videos, 1):
        ...
        # ŽANR JE UVIJEK "Akcija" ZA NOVE FILMOVE
        ...
        new_movies_count += 1
        print(f"   ✅ DODAT U XML: '{cleaned_title}' (Žanr: Akcija)")
    
    # Snimi NOVI XML SA LEPŠIM FORMATIRANJEM
    tree = ET.ElementTree(new_root)
    output_file = "akcija_1.xml"
    ...
    print(f"   • Postojećih filmova: {existing_count}")
...
if __name__ == "__main__":
    ...
    print("3. Ažuriraj žanr za sve filmove na 'Akcija'")
    
    choice = input("\nOdaberite opciju (1, 2 ili 3): ").strip()
    
    if choice == "2":
        process_existing_titles()
    elif choice == "3":
        update_genre_to_akcija()
    else:
